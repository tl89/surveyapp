...
<html lang="en">
  ...
  <body>
	...
	<div class="container-fluid surv-quest">
		<h1 class="text-center">Edit Survey Code</h1>
		<form method="post" action="/SURVEYAPP/classes/admin/survCodeDisp.cfc?method=changeSurvCode">
			<p class="input">
				<div class="form-group row">
					<div class="col-xs-6">
						<label for="survCode">Survey Code: </label>
						<input class="form-control" name="survCode" id="survCode" value="<cfoutput>#url.surveyCode#</cfoutput>" type="text" readonly="readonly">
					</div>
					<div class="col-xs-6">
						<label for="codeAvail">Uses available: </label>
						<input class="form-control" name="codeAvail" id="codeAvail" value="<cfoutput>#url.codeAvail#</cfoutput>" type="text">
					</div>
				</div>
			</p>
			
			<p class="text-center" style="padding-top:5%;">
				<input type="submit" name="submit" id="submit" value="Done" class="btn btn-primary" style="width:40%;"/><br/>
