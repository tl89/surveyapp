(function($) {
		...
		jQuery.fn.showState = function(page) { 
			...
			if (settings.stateFunction) { 
				$('h5').text('Progress (' + curpage + '/' + settings.pages.length + ')');
				$('#innerProg').animate({'width': (curpage / settings.pages.length) * 100 + '%'}, 1000);
	...
	jQuery.fn.multipage = function(options) { 
		...
		settings = jQuery.extend({stayLinkable:false,submitLabel:'Submit',hideLegend:false,hideSubmit:false,generateNavigation:false,activeDot:'&nbsp;&#x25CF;',inactiveDot:'&nbsp;&middot;'},options);
		id = '#' + $(this).attr('id');
		var form = $(this);			
